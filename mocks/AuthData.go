// Code generated by mockery v2.14.0. DO NOT EDIT.

package mocks

import (
	auth "lami/app/features/auth"

	mock "github.com/stretchr/testify/mock"
)

// AuthData is an autogenerated mock type for the Data type
type AuthData struct {
	mock.Mock
}

// FindUser provides a mock function with given fields: param
func (_m *AuthData) FindUser(param string) (auth.Core, error) {
	ret := _m.Called(param)

	var r0 auth.Core
	if rf, ok := ret.Get(0).(func(string) auth.Core); ok {
		r0 = rf(param)
	} else {
		r0 = ret.Get(0).(auth.Core)
	}

	var r1 error
	if rf, ok := ret.Get(1).(func(string) error); ok {
		r1 = rf(param)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

type mockConstructorTestingTNewAuthData interface {
	mock.TestingT
	Cleanup(func())
}

// NewAuthData creates a new instance of AuthData. It also registers a testing interface on the mock and a cleanup function to assert the mocks expectations.
func NewAuthData(t mockConstructorTestingTNewAuthData) *AuthData {
	mock := &AuthData{}
	mock.Mock.Test(t)

	t.Cleanup(func() { mock.AssertExpectations(t) })

	return mock
}
